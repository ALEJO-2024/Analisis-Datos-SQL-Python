...
/*Ejercicio 2: Creación y Uso de una Vista de Películas
Objetivo: Crear una herramienta reutilizable para analizar el catálogo de películas. Punto de Partida: 
Revisa los JOINs que utilizaste en el Ejercicio 3 del Taller de JOINs (Películas por Categoría).
Tarea:
1. Crea una vista llamada vista_film_detalles que contenga el título (title), 
el idioma (name de language) y la categoría (name de category) de cada película.

2. Una vez creada la vista, úsala para responder a la pregunta: ¿Cuántas películas de la categoría 'AVENTURA' están en 'English'?
*/

Create view vista_film_detalles as
Select f.title 'Nombre_pelicula', l.name 'Idioma', c.name 'Categoria'
From film f
...
Select count(*) 'Numero_peliculas'
From vista_film_detalles
where Categoria = 'AVENTURA' and Idioma = 'English';

select * from vista_film_detalles;

/* Ejercicio 3: Top 5 Actores en Películas de Categoría 'Family'

Objetivo: Combinar CTEs y JOINs para un análisis específico.

Punto de Partida: Utiliza la lógica del Ejercicio 4 del Taller de JOINs (Actores más Prolíficos) como base.

Tarea: Adapta esa consulta para que devuelva el Top 5 de actores que han aparecido en más películas, 
pero únicamente de la categoría 'Family'. El resultado debe mostrar el nombre completo del actor y la cantidad de películas.
...
WITH TOP_ACTORES AS (
    SELECT
        ...
        CONCAT(ac.first_name, ' ', ac.last_name) AS nombre_completo,
